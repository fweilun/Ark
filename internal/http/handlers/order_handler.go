...
func (h *OrderHandler) Match(w http.ResponseWriter, r *http.Request) {
	id := r.PathValue("id")
	if id == "" {
		writeError(w, http.StatusBadRequest, "missing order id")
		return
	...
	err := h.order.Match(r.Context(), order.MatchCommand{
		OrderID:  types.ID(id),
	...
	writeJSON(w, http.StatusOK, map[string]any{"status": order.StatusDriverFound})
}
...
func (h *OrderHandler) Pay(w http.ResponseWriter, r *http.Request) {
	...
	err := h.order.Pay(r.Context(), order.PayCommand{OrderID: types.ID(id)})
	if err != nil {
		writeOrderError(w, err)
		return
	}
	writeJSON(w, http.StatusOK, map[string]any{"status": order.StatusPayment})
}
