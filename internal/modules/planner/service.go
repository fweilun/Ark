...
// ErrNotFound is returned when a requested plan does not exist.
var ErrNotFound = errors.New("plan not found")

// ErrBadRequest is returned when required command fields are missing or invalid.
...
// UpdatePlanCommand contains the fields that may be modified on an existing plan.
...
type Service interface {
	// CreatePlan creates a new draft plan for a passenger.
	CreatePlan(ctx context.Context, cmd CreatePlanCommand) (*Plan, error)

	// GetPlan retrieves a plan by its ID.
	GetPlan(ctx context.Context, id string) (*Plan, error)

	// UpdatePlan modifies origin, destination, pickup time, status, or notes.
	UpdatePlan(ctx context.Context, cmd UpdatePlanCommand) (*Plan, error)

	// DeletePlan removes a plan permanently.
	DeletePlan(ctx context.Context, id string) error

	// ListPlans returns all plans for a given passenger ordered by pickup time.
...
func (s *plannerService) UpdatePlan(ctx context.Context, cmd UpdatePlanCommand) (*Plan, error) {
	...
	// Apply only provided fields (zero-value means "no change").
	if cmd.Origin != "" {
...
// newPlanID generates a 32-char hex random ID.
